...
public class RewardsBox extends Random {
    ...
    private Reward getSelectedReward() {
        ...
        if (reward != null) {
            for (Reward r : Reward.values()) {
                if (r.name().equalsIgnoreCase(reward) && (interfaces.get(Values.INTERFACE_MAIN_BOX).isValid() ? (getRewardInterface(r).isValid()) : true)) {
                    return r;
                }
            }
        }
    ...
    protected int loop() {
        ...
        if (SELECTED_REWARD == null && (interfaces.get(Values.BOX_ID).isValid() || interfaces.get(Values.INTERFACE_XP_SELECTION).isValid())) {
